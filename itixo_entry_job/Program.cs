...
namespace itixo_entry_job
{
    public class Program
    {
        ...
        static async Task<string> DownloadXmlAsJson(string url)
        {
        ...
        private static async Task ProcessAndSaveData(RootObject? root)
        {
        ...
        static async Task Main(string[] args)
        {
            ...
            while (true)
            {
                string jsonString = await DownloadXmlAsJson(xmlUrl);

                RootObject? root = JsonConvert.DeserializeObject<RootObject>(jsonString);
                await ProcessAndSaveData(root);
